...
describe('<Faq />', () => {
  ...
  it('should escape unsafe HTML content, when rendering the markdown', async () => {
    const mocks = [
      {
        request: { query: FAQ_DATA_QUERY, variables: { id: 0 } },
        result: {
          data: {
            faq: {
              title: 'faq title in here!',
              body: 'faq body content in here! <script>console.log("moo-cow")</script>',
              __typename: 'faq',
            },
          },
        },
    ...
    expect(fixture.find('[data-test-reference="faq-body"]').find('script').exists()).toBeFalsy();
  });
});
